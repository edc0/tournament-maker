...
class TournamentData:
    ...
    def new_tournament(self, player_list):
        ...
        self.df_teams = pd.DataFrame([], columns = ['ROUND_NUM', 'PLAYER1_ID', 'PLAYER2_ID', 'PLAYER3_ID'])
        self.df_games = pd.DataFrame([], columns = ['ROUND_NUM', 'TEAM1_ID', 'TEAM2_ID', 'GOALS_TEAM1', 'GOALS_TEAM2'])
        self.df_players_activity = pd.DataFrame([], columns = ['ROUND_NUM', 'PLAYER_ID', 'ACTIVE'])
    ...
    def load_tournament(self):
        ...
        self.df_players_activity = pd.read_csv(f"{self.path}/players_activity.csv", index_col=0)
    ...
    def initialize_player_round_activity(self, round_num):
        for player_id, _ in self.df_players.itertuples():
            ...
            self.df_players_activity.loc[f"{player_id}R{round_num}"] = [round_num, player_id, activity]

    def set_player_active(self, player_id, round_num):
        self.df_players_activity.loc[f"{player_id}R{round_num}"] = [round_num, player_id, True]
    
    def set_player_inactive(self, player_id, round_num):
        self.df_players_activity.loc[f"{player_id}R{round_num}"] = [round_num, player_id, False]
    ...
    def add_player(self, player_name):
        player_id = 0 if len(self.df_players) == 0 else self.df_players.index.max() + 1
        self.df_players.loc[player_id] = [player_name, str(True)]
        self.save_players()
        return player_id

    def add_team(self, round_num, player1_id, player2_id, player3_id):
        team_id = 0 if len(self.df_teams) == 0 else self.df_teams.index.max() + 1
        self.df_teams.loc[team_id] = [round_num, player1_id, player2_id, player3_id]
        self.save_teams()
        return team_id

    def add_game(self, round_num, team1_id, team2_id, goals_team1=None, goals_team2=None):
        game_id = 0 if len(self.df_games) == 0 else self.df_games.index.max() + 1
        self.df_games.loc[game_id] = [round_num, team1_id, team2_id, goals_team1, goals_team2]
        self.save_games()
        return game_id
    ...
    def calculate_ranking(self):
        df_ranking = pd.DataFrame([], columns = ["PLAYER_NAME", "WINS", "LOSSES", "TIES", "PLAYED_GAMES", "GOALDIFF", "POINTS"])
        ...
        for _, _, team1_id, team2_id, goals_team1, goals_team2 in self.df_games.itertuples():
            ...
            [team1_result_identifier, team2_result_identifier] = _get_result_identifier(goals_team1, goals_team2)
            ...
            for player_id in player_IDs_team1:
                if player_id != TournamentData.SUB:
                    ...
                    df_ranking.loc[player_id, ["GOALDIFF"]] += int(goals_team1-goals_team2)
            ...
            for player_id in player_IDs_team2:
                if player_id != TournamentData.SUB:
                    ...
                    df_ranking.loc[player_id, ["GOALDIFF"]] += int(goals_team2-goals_team1)
                    df_ranking.loc[player_id, ["PLAYED_GAMES"]] += 1
            
        # Calculates the points for each player
        df_ranking["POINTS"] = df_ranking["WINS"] * 3 + df_ranking["TIES"]
